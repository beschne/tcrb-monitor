...
Ad-hoc helper scripts that support the main T CrB monitoring/photometry workflow but aren't part of it directly — standalone, run manually, not imported by anything else in this repo.
...
```bash
python3 tools/tcrb_session_midtime.py                 # scan every "<date> lights" folder in the default campaign dir
python3 tools/tcrb_session_midtime.py "/path/to/some lights folder"   # or scan specific folder(s)
```
